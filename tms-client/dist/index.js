...
const enrollment_model_2 = require("./models/enrollment.model");
...
const course = {
    ...
    capacity: 50,
    startDate: polyfill_1.Temporal.PlainDate.from("2025-01-13"),
};
...
const invalidStudentData1 = 42; // A number, not an object
const invalidStudentData2 = {
    id: 12345, // Wrong type: number instead of string
    name: "Test",
};
// TEST 1: Valid data with type guard
console.log("Test 1: Valid student data");
if ((0, student_model_1.isStudent)(validStudentData)) {
...
if ((0, student_model_1.isStudent)(invalidStudentData1)) {
    console.log("  ✓ Valid");
}
else {
...
if ((0, student_model_1.isStudent)(invalidStudentData2)) {
    console.log("  ✓ Valid");
}
else {
...
try {
    const parsedStudent = (0, student_model_1.parseStudent)({
        id: "STU-001",
        name: "Hana Tadesse",
        enrollmentDate: polyfill_1.Temporal.Now.instant().toString(),
    });
    console.log(`  ✓ Successfully parsed: ${parsedStudent.name} (${parsedStudent.id})\n`);
}
catch (error) {
...
try {
    (0, student_model_1.parseStudent)({
        id: 12345, // Wrong type
        name: "Test",
        enrollmentDate: polyfill_1.Temporal.Now.instant().toString(),
    });
...
if ((0, course_model_1.isCourse)(validCourseData)) {
    console.log(`  ✓ Valid: ${validCourseData.title} (${validCourseData.capacity} seats)`);
}
else {
...
if ((0, course_model_1.isCourse)(invalidCourseData)) {
    console.log("  ✓ Valid");
}
else {
...
const validEnrollmentData = {
    studentId: "STU-001",
    courseCode: "CSE-10001",
    enrolledAt: polyfill_1.Temporal.Now.instant().toString(),
};
...
if ((0, enrollment_model_1.isEnrollmentRecord)(validEnrollmentData)) {
    console.log(`  ✓ Valid: ${validEnrollmentData.studentId} -> ${validEnrollmentData.courseCode}`);
}
else {
...
if ((0, enrollment_model_1.isEnrollmentRecord)(invalidEnrollmentData)) {
    console.log("  ✓ Valid");
}
else {
    console.log("  ✓ Guard correctly rejected: studentId must be string\n");
}
// ============================================================================
...
console.log(`  Grade: ${(0, assessment_model_1.calculateGrade)(quiz)}%`); // 80
console.log(`Lab: ${lab.title}`);
console.log(`  Grade: ${(0, assessment_model_1.calculateGrade)(lab)}%`); // 87
...
console.log(`Enrollment Status: ${(0, enrollment_model_2.describeEnrollment)(pending)}\n`);
const active = {
    status: "ACTIVE",
    startDate: polyfill_1.Temporal.PlainDate.from("2026-09-01"),
    currentGrade: 87,
};
console.log(`Enrollment Status: ${(0, enrollment_model_2.describeEnrollment)(active)}`);
...
console.log(`Course Status: ${(0, course_model_2.describeCourse)(activeCourse)}\n`);
const draftCourse = {
    status: "DRAFT",
    createdBy: "dr.smith",
    createdAt: polyfill_1.Temporal.Now.instant(),
};
console.log(`Course Status: ${(0, course_model_2.describeCourse)(draftCourse)}`);
...
const courseListRes = {
    ...
    data: [
        ...
        },
        {
            ...
            capacity: 25,
            startDate: polyfill_1.Temporal.PlainDate.from("2026-09-15"),
        },
    ],
    fetchedAt: polyfill_1.Temporal.Now.instant(),
};
...
console.log("\nError Response:");
console.log(`  ${(0, api_response_model_1.renderResponse)(errorRes, (_s) => "")}`); // Formatter is never called in error state
...
const courseStart = polyfill_1.Temporal.PlainDate.from("2026-09-15");
